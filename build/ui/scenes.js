    ...
    game: {
        ...
        display(currentFrame) {
            ...
            for (const ctx of Object.values(CTX)) {
                if (!(ctx == CTX.tiles || ctx == CTX.tilesOver) || currentFrame.redraw) {
                    Gfx.clear(null, ctx);
                }
            }
            ...
            Game.level1.displayGhostBuilding(...(Camera.unproject(...Input.mouse).map(Pos.pixelToTile)), Input.placedBuilding.ID, currentFrame);
        ...
        onmousedown(e) {
            ...
            if (e.ctrlKey && e.button == 0) {
                Game.level1.buildingAtPixel(...(Camera.unproject(e.x, e.y)))?.acceptItem(new Item(...(Camera.unproject(e.x, e.y).map(c => Pos.tileToPixel(Pos.pixelToTile(c), true))), "base_null"), null);
            }
            else if (e.button === 1) {
