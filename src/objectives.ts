...
*/
/* Contains the objective system. */

import { crash, sort2 } from "./util/funcs.js";
import type { NonEmptyArray } from "./util/types.js";
...
export const tech = new TechTree(() => Game.level1, tree => {
	...
	const stirling_generator = tree.node("building_base_stirling_generator", [["base_ironIngot", 50], ["base_coal", 25], ["base_copperIngot", 30]], [alloy_smelter]);
	const compressor = tree.node("building_base_compressor", [["base_ironIngot", 50], ["base_steelIngot", 10], ["base_copperIngot", 50]], [stirling_generator]);
	const lathe = tree.node("building_base_lathe", [["base_ironIngot", 50], ["base_steelIngot", 15], ["base_copperIngot", 50]], [stirling_generator]);
	const wiremill = tree.node("building_base_wiremill", [["base_ironIngot", 50], ["base_steelIngot", 10], ["base_copperIngot", 50]], [stirling_generator]);
	const pipe = tree.node("building_base_pipe", [["base_ironPlate", 20]], [compressor]);
...
export const objectives = new ObjectiveList(() => {
	...
	const activateSteamGenerator = new Objective("base_activateSteamGenerator", [researchSteamGenerator], () => Game.level1.grid.producers.some(p => p.block.id == "base_steam_generator" && (p as BuildingWithRecipe).efficiencyp > 0));
